...
function dragContainerSidebar(container, categories) {
    ...
    container.querySelectorAll(".draggable-item").forEach((item) => {
        item.addEventListener("dragstart", (e) => {
            ...
            // Mẹo: Xóa padding trực tiếp để trình duyệt chụp snapshot "sạch"
            item.style.paddingLeft = "15px";

            // Dùng setTimeout để thêm class dragging sau khi trình duyệt đã lấy ảnh snapshot
            // Điều này giúp item gốc mờ đi nhưng cái bóng (ghost image) đã kịp chụp lúc padding = 15px
            setTimeout(() => {
